...
# copies the star's side of the assistant (pages_helper.py, the two badges, the two apps)
...
if [ -d "$STAR" ]; then
    mkdir -p "$HERE/star"
    for rel in overlay/pages_helper.py overlay/hdbadge.lua overlay/pagebadge.lua apps/display.lua apps/pages.lua; do
        if [ -f "$STAR/$rel" ]; then
